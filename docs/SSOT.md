...
## 목록 필터(Zone)와 검색(q) 우선순위 규칙 (확정)

현장 사용성 기준으로, Zone 버튼(냉동1/냉동2/냉장/상온)은 **카테고리 훑어보기(브라우징)** 용도이며, **검색을 좁히기 위한 목적이 아니다.**

- **검색어 q가 비어 있을 때만** Zone 필터를 적용한다.
  - zone=냉동1/냉동2/냉장/상온 중 하나가 선택되면 해당 Zone만 목록에 노출한다.
  - **전체** 버튼은 zone 필터를 해제한다(= URL에서 `zone` 제거).

- **검색어 q가 존재하면** Zone 필터는 **무시한다.**
  - 어떤 Zone 버튼이 선택되어 있어도 검색 결과는 **전체 상품**(active=true)에서 나온다.
  - 검색 대상은 **상품명(name) + 제조사(manufacturer)**(및 SSOT에 정의된 추가 필드)이며, 토큰(쉼표/공백) 기반 AND 매칭을 사용한다.
  - 검색창에 Zone 키워드(냉동1 등)를 해석하는 예외 로직은 두지 않는다.

### 상태 저장(필수)
- 목록 상태(선택된 zone, 검색 q)는 **URL query params로만** 저장한다.
  - 예: `/products?zone=냉동1&q=돈까스`
- localStorage 기반 상태 저장은 금지한다.

### IME(한글 입력) 안정성(필수)
- 검색 입력 중 URL을 매 키 입력마다 갱신하지 않는다.
- 한글 조합(composition) 중에는 `router.replace` 등 네비게이션성 업데이트를 금지한다.
- q의 URL 반영은 composition 종료 또는 debounce(약 300ms)로 처리한다.
